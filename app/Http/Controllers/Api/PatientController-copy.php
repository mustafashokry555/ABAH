    ...
    function addTheAppoint(Request $request)
    {
        ...
        if ($user) {
            ...
            $row = array_merge($row, [
                ...
                "Middlename" => $request->has('Middlename') ? $request->input('Middlename') : null,
                "LastName" => $request->has('LastName') ? $request->input('LastName') : null,
                "Age" => $request->Age,
                "Gender" => $request->has('Gender') ? $request->input('Gender') : null,
                "StreetAddress" => $request->has('StreetAddress') ? $request->input('StreetAddress') : null,
            ...
            try {
                $newAppointment = DB::table('Ds_PatientAppoinmentTemperary')
                    ->insert([$row]);
                // DB::insert("INSERT INTO Ds_PatientAppoinmentTemperary 
                // (ID, FromTime, ToTime, AppointmentCode, DoctorID, Duration, Loc_Id, app_date,
                // PatientID, Firstname, Middlename, LastName, Age, Gender, StreetAddress, City, State, Mobile, Country)
                // VALUES (
                //     $row[ID],
                //     $row[FromTime],
                //     $row[ToTime],
